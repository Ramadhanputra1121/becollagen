...
An API built with NestJS 9.x and Passport Auth JWT System :heart_eyes_cat:

## Installation

Install the latest NodeJS from https://nodejs.org/en/download/
Install MySQL. Then you need to run the database.

Go to project repository and open terminal then execute following command:
...
```bash
   ts-node node_modules/.bin/typeorm migration:run -d typeorm-cli.config
```

or

```bash
   node_modules/.bin/typeorm migration:run -d typeorm-cli.config
```
